...
| 2026-03-25 | Responsive design for all screen sizes. Navbar: hamburger menu on mobile, hidden logo text/name, mobile dropdown panel. Calendar: grid hidden on mobile (`hidden sm:block`), list view is primary on small screens. Admin: stacked header/controls, responsive tabs (`text-xs sm:text-sm`), horizontal scroll tables (`min-w-[600px]`/`min-w-[640px]`). MasterCalendar: reduced padding, legend labels hidden on mobile. ImportPanel: stacked clear controls. All using Tailwind `sm:` breakpoint (640px). | `Navbar.tsx`, `CalendarGrid.tsx`, `DayCell.tsx`, `AdminPage.tsx`, `AssignmentTable.tsx`, `UserManager.tsx`, `ShiftLimitsManager.tsx`, `MasterCalendar.tsx`, `ImportPanel.tsx` |
| 2026-03-25 | Mobile touch scroll fixes: Overview Daily Coverage and All Schedules tables use `overscroll-behavior: contain` to prevent bouncing past edges. All Schedules Agent column uses `touch-action: pan-y` (vertical swipe only), data cells use `touch-action: pan-x` (horizontal swipe only). Shift Limits outer container prevents horizontal scroll of top section (`overflow-x-hidden`). Shift Limits Per-Agent table: Agent column is `sticky left-0` with `touch-action: pan-y`, data columns use `touch-action: pan-x`, header Agent cell also sticky. Agent column gets solid background colors to cover content when scrolling. | `MasterCalendar.tsx`, `ShiftLimitsManager.tsx` |
| 2026-03-25 | Fix invisible loading spinner: changed from `border-primary` on light bg (nearly invisible) to white spinner on dark brand background (`#1a1a3e`), so loading state is clearly visible instead of appearing as a blank page. | `App.tsx` |
| 2026-03-31 | Mobile calendar list view redesign: colored rows by day type background, bold day type labels (`text-base font-bold`), wider claim dropdown (`w-24` chevron-only trigger), "No bonus"/"All claimed" status hints via `emptyText` prop on ShiftDropdown, dropdown disappears when limits reached (removed "Limit reached" text), removed unused Badge import. Desktop grid unchanged. | `CalendarGrid.tsx`, `ShiftDropdown.tsx` |
| 2026-04-02 | Overview page fixes: (1) Daily Coverage `overscroll-behavior-x: none` + matching bg to prevent white gap on mobile horizontal scroll. (2) All Schedules touch direction locking (JS-based axis lock on touchmove) to prevent scroll axis interference on mobile. (3) All Schedules `overscroll-behavior: none` + matching bg to prevent white gap. (4) All Schedules cells now show both base shift and bonus shift (e.g., "N" + "EB" badge) instead of replacing base with bonus — only `W` is replaced. Excel export updated to match. | `MasterCalendar.tsx`, `exportXlsx.ts` |
| 2026-04-09 | Fix simultaneous multi-user logout: Supabase defaulted to a single shared `localStorage` key and a shared `BroadcastChannel`, causing any new login to overwrite all other active sessions. Now uses a per-tab unique `storageKey` backed by `sessionStorage`, fully isolating each tab's session and BroadcastChannel. Sessions survive page refreshes and end when the tab is closed. | `src/lib/supabase.ts` |
